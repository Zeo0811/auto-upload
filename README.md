...
- 自动填写标题、描述、话题标签
- 自定义封面上传（小红书支持比例选择，视频号支持个人卡片 + 分享卡片，抖音支持自定义封面）
- 定时发布（日期 + 时间选择器）
...
| `tags` | list[str] | 话题标签 | 全部 |
| `cover_path` | string | 封面图片路径 | 全部 |
| `cover_ratio` | string | 封面比例，如 "3:4" | 小红书 |
| `publish_time` | string | 定时发布，格式 "YYYY-MM-DD HH:MM" | 全部 |
